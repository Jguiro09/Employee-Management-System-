...
async function employeeViewManager (){
    let choice = await tableData.getManagers();
    inquirer.prompt([
        {
            type: 'list',
    ...
    ]).then((answer) => {e.viewEmployeeByManager(answer.manager ,initialQuestions);})
}

async function employeeViewDepartment(){
    let choice = await tableData.getDepartments();
    ...
    .then((answer) => {e.viewEmployeeByDepartments(answer.department, initialQuestions)})
}
